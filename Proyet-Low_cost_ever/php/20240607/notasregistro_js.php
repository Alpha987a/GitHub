    ...
    document.addEventListener('DOMContentLoaded',function(){
        ...
        .then(data=>{
            data.forEach(alumno=>{
                ...
                option.textContent = alumno.apellidos + " " + alumno.nombre;
                alumnoSelect.appendChild(option);
            });
        }).catch(error =>console.log('Error en alumnos data: ', error));
    });

    //escuchar el evento change en la lista de alumnos y completar lista cursos
    alumnoSelect.addEventListener('change', function(){
        cursoSelect.textContent = "";
        alumnoId = alumnoSelect.value;
        ...
        .then(data=>{
            data.forEach(alumno=>{
                if(alumno.id == alumnoId){
                    const option = document.createElement('option');
                    option.value =alumno.cursoId ;
                    option.textContent = alumno.curso ;
                    cursoSelect.appendChild(option);
                }


            });
        }).catch(error =>console.log('Error en cursos data: ', error));
    });
